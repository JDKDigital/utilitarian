    ...
    public static void onLoad(ScreenEvent.Init.Post event) {
        if (!hasDinged && (event.getScreen() instanceof AccessibilityOnboardingScreen || event.getScreen() instanceof TitleScreen)) {
            if (!ModList.get().isLoaded("ding") && Config.DING_DONG_ENABLED.get()) {
                Identifier soundName = Identifier.parse(Config.DING_DONG_SOUND.get());

                Minecraft.getInstance().getSoundManager().play(new SimpleSoundInstance(soundName, SoundSource.MASTER, 1.0f, 1.0f, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true));
                hasDinged = true;
            }
        }
    }
